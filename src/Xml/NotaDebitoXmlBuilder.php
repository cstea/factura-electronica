...
/**
 * Builds the (unsigned) body of a Nota de Débito Electrónica (ND, tipo 02).
 *
 * Schema-valid only; not byte-verified against a real accepted ND (no sample available).
 */
final class NotaDebitoXmlBuilder implements DocumentBuilder
{
    use BuildsDomElements;

    private const NS = 'https://cdn.comprobanteselectronicos.go.cr/xml-schemas/v4.4/notaDebitoElectronica';

    public function build(object $dto, string $clave): DOMDocument
    {
        if (! $dto instanceof NotaDebitoDto) {
            throw new XmlBuildException('NotaDebitoXmlBuilder requires a NotaDebitoDto.');
        }

        $doc = new DOMDocument('1.0', 'utf-8');
        $doc->formatOutput = false;

        $root = $doc->createElementNS(self::NS, 'NotaDebitoElectronica');
    ...
    private function buildLineaDetalle(DOMDocument $doc, DOMElement $detalle, LineaDetalleDto $linea): void
    {
        ...
        // BaseImponible is required in the ND XSD (no minOccurs="0").
    ...
    private function buildResumenFactura(DOMDocument $doc, DOMElement $root, NotaDebitoDto $dto): void
    {
    ...
    private function buildInformacionReferencia(DOMDocument $doc, DOMElement $root, InformacionReferenciaDto $ref): void
    {
        ...
        $this->el($doc, $node, 'FechaEmisionIR', $ref->fechaEmisionIR->format('Y-m-d\TH:i:sP'));
    ...
    {
        return dirname(__DIR__, 2).'/resources/xsd/v4.4/notaDebitoElectronica.xsd';
    }
}
