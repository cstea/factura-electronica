...
/**
 * Builds the (unsigned) body of a Nota de Crédito Electrónica (NC, tipo 03).
 *
 * Schema-valid only; not byte-verified against a real accepted NC (no sample available).
 */
final class NotaCreditoXmlBuilder implements DocumentBuilder
{
    ...
    public function build(object $dto, string $clave): DOMDocument
    {
        if (! $dto instanceof NotaCreditoDto) {
            throw new XmlBuildException('NotaCreditoXmlBuilder requires a NotaCreditoDto.');
        }

        $doc = new DOMDocument('1.0', 'utf-8');
        $doc->formatOutput = false;

        $root = $doc->createElementNS(self::NS, 'NotaCreditoElectronica');
    ...
    private function buildLineaDetalle(DOMDocument $doc, DOMElement $detalle, LineaDetalleDto $linea): void
    {
        ...
        // BaseImponible is required in the NC/ND XSD (no minOccurs="0").
    ...
    private function buildResumenFactura(DOMDocument $doc, DOMElement $root, NotaCreditoDto $dto): void
    {
    ...
    {
        return dirname(__DIR__, 2).'/resources/xsd/v4.4/notaCreditoElectronica.xsd';
    }
}
